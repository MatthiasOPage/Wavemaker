...
timeSplit = period/3;                                                                                              //A ratio precomputed for efficiency
maxSpd = abs(floor((2*amplitude_step)/timeSplit));                                                                           //Setting stepper speed in steps/second
maxAccel = abs(floor((2*amplitude_step)/(timeSplit*timeSplit)));                                                             //Setting stepper acceleration in steps/(second)^2
...
currentSpeed = maxSpd;                                                                                             //Initializing a variable for recursive speed calibration
currentAccel = maxAccel;                                                                                           //Initializing a variable for recursive acceleration calibration
timeScale = 0;                                                                                                     //Initializing timescale
i = 1;                                                                                                                //Simple boolean integer to skip first oscillation
halfPeriodMillis = 0.5*(period*1000);                                                                              //Target halfperiod in milliseconds
loopStartTime = 0;                                                                                          //Variable used to store when the movement starts
loopStopTime = 0;                                                                                           //Variable used to store when the movement ends
deltaTime = 0;                                                                                              //Variable used to store time difference
Calibration = false;

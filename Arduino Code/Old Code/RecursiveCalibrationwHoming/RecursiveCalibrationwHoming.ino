...
#define home_switch 9                                                                                                     //Attach limit switch to pin 9
...
double Amp = 10;                                                                                                          //Wavemaker amplitude in cm
double Period = 2.5;                                                                                                      //Wave period in seconds
double microSteps = 4;                                                                                                    //Microstep setting on stepper driver
double Center = 10;                                                                                                       //Center position in cm down from home
...
AccelStepper stepper(1, 3, 6);                                                                                            //Stepper 1 with 3-PUL and 6-DIR
...
//Loop with recursive ratio controlled speed calibration
void loop()
{
    if (stepper.distanceToGo() == 0){                                                                                     //If at the end of the movement
      ...
      deltaTime = loopStopTime-loopStartTime;                                                                             //Calculating the time taken barring the first loop
          if (i != 1 && abs(deltaTime-halfPeriodMillis) >= 2){                                                            //If not on the first loop and not accurate period
              ...
              Serial.println("Calibrating...");                                                                           //Let the user know the movement is still being adjusted

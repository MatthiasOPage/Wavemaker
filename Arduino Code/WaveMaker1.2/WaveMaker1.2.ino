...
////Wavemaker Properties////
  double limit_distance = 22.855; //[cm] should be 5 sig-figs

////Tolerance Properties////
  double position_resolution = 25; //[pulses] note, rotary encoder has 400 pulses per rev
  double step_resolution = 25; //[steps] 
  double period_resolution = 10; //[ms]

////Calibration Properties////
  double homing_delay = 1; //[ms]
  double homing_speed = 100000; //[steps/s] actual movement will be slower due to the homing_delay
  double homing_accel = 100000; //[steps/s^2]
  int iteration_max = 30; //max time the calibration loop will try to fine tune movement before throwing error
  double ratio = 0.3333333; //how much of the period is spent on accelerating, movement, deccelerating
...
////Hardware Declerations////
  AccelStepper stepper(1, 5, 6); //step on dir on pin 5, pul on pin 6
  Encoder rotary_encoder(2, 3); //must be on interrupt pins (only 2 and 3 for UNO)
